...
def store_analytics(user_id, analysis, events_summary):
    ...
    exit_risk_score = analysis.get('exit_risk_score', 0)
    ...
    item = {
        ...
        'exitRiskScore': Decimal(str(exit_risk_score)),
        'recommendedActions': analysis.get('recommended_actions', []),
        'eventsAnalyzed': events_summary,
        'interventionTriggered': False,  # Will be updated if intervention is triggered
    ...
    # Log high-risk detection (frontend will poll and show popup)
    if exit_risk_score >= 75:
        print(f"🚨 High exit risk detected ({exit_risk_score}) for user {user_id} - intervention available for frontend")
